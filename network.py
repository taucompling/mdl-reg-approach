...
def _gaussian(x):
    # Gaussian with mean 0 and sigma 1. Taken from Gaier & Ha.
...
def get_weight_real_value(w: _Weight) -> float:
    return w.sign * w.numerator / w.denominator


def _get_next_available_unit(units: frozenset[_Node]) -> int:
    """
    Units = {0,1,2,3} -> 4
    Units = {0,1,2,5} -> 3
    """
    try:
        return next(x for x in range(len(units)) if x not in units)
...
def topological_sort_through_time(
    ...
    """Topological sort which also takes into account units that are only connected to the network through recurrent edges (i.e. not fed by inputs).
    This makes it possible to:
    1. Evolve no-input sub-circuits that connect to the network through recurrent edges, e.g. unit 2 in example below.
    2. Detect loops that are only reachable through recurrent edges.
    ```digraph {
        0 [label="input"]
        1 [label="input"]
        4 [label="output"]
    ...
    units = get_units(net)
    loop_edges_for_forward_paths, topological_sort = dfs(
        units=units,
    ...
    loop_edges_including_recurrent_paths, recurrent_topological_sort = dfs(
        units=units,
        reverse_connections=net.reverse_forward_connections,
        input_units=net.input_units_range,
...
def bfs_layers(
    ...
    # Returns a dict {layer depth: set of units in depth}. Assumes no loops.
    entry_node = _get_next_available_unit(units)
    sink_node = _get_next_available_unit(units | {entry_node})

    connections = _dict_copy(forward_connections)
    connections[entry_node] = frozenset(input_units_range)
    for output_unit in output_units_range:
        if output_unit not in connections:
            connections[output_unit] = set()
        connections[output_unit] |= {sink_node}

    hidden_units_without_inputs = (
        units - set(input_units_range) - set(reverse_forward_connections)
    )
    connections[entry_node] |= hidden_units_without_inputs

    node_to_depth = {}

    queue = [(-1, entry_node)]
    while queue:
        depth, current_node = queue[0]
        queue = queue[1:]
        neighbors = list(connections.get(current_node, set()))
        for neighbor in neighbors:
            queue.append((depth + 1, neighbor))
            node_to_depth[neighbor] = max(depth + 1, node_to_depth.get(neighbor, 0))

    depth_to_nodes = collections.defaultdict(set)
    for node, depth in node_to_depth.items():
        if node not in {entry_node, sink_node}:
            depth_to_nodes[depth].add(node)

    return {depth: frozenset(depth_to_nodes[depth]) for depth in depth_to_nodes}
...
def dfs(
    ...
    Returns:
        - Set of loop-closing edges (non-unique)
        - Topological sort of nodes in the graph as if it didn't contain cycles.
    """
...
def _geometric_sample(shape=None, minimum: int = 0, dtype=int):
...
@functools.lru_cache(maxsize=100_000)
def _get_weight_encoding(weight: _Weight) -> str:
    encoding = ""
...
def _get_bitsring_encoding(net: Network, allowed_activations: tuple[int, ...]) -> str:
...
def get_encoding_length(
    ...
    if compress_encoding:
        bytes_ = utils.compress_string(encoding_bitstring)
        return len(bytes_)
...
def _calculate_bias(node: int, node_bias: _Weight, node_values: dict, unit_type: int):
    node_values[node] = node_values[node] + get_weight_real_value(node_bias)
...
def _get_incoming_units_and_weights(
    ...
    for node in topological_sort:
        ...
        forward_weights = np.array(
            [
                get_weight_real_value(net.forward_weights[(from_node, node)])
        ...
        if incoming_recurrent_units:
            ...
            recurrent_weights = np.array(
                [
                    get_weight_real_value(net.recurrent_weights[(from_node, node)])
...
def predict_probabs(
    ...
    sequence_outputs = feed_sequence(
        ...
        try_net_early_stop=try_net_early_stop,
...
def calculate_deterministic_accuracy(
    ...
    output_probabs = predict_probabs(
        ...
        try_net_early_stop=config.try_net_early_stop,
    ...
    if corpus.sample_weights:
        rows_repeated_by_weights = list(
            itertools.chain(
                *[
                    # Since weights can sometimes be float probabilities, we can't just multiply by them.
                    # If the weight is probability, this means underlyingly that we can treat the sequence as appearing
                    # just once for the accuracy (for loss purposes we must use the probability). This perhaps can be
                    # improved in the future.
                    [row] * (weight if isinstance(weight, int) else 1)
                    for row, weight in enumerate(corpus.sample_weights)
                ]
            )
        )
...
def _get_sequence_encoding_length(
    ...
    softmax_outputs: bool,
    try_net_early_stop: bool,
    data_given_grammar_smoothing_epsilon: Optional[float],
    ...
    train_test_overlap_mask: Optional[np.ndarray],
) -> tuple[float, float]:
    output_probabs = predict_probabs(
        net=net,
    ...
    if not allow_test_overlap:
        if train_test_overlap_mask is None:
            raise RuntimeError(
                "Test overlap is not allowed, but no train-test overlap mask is given."
            )

        output_probabs = output_probabs[~train_test_overlap_mask]
        target_sequence = target_sequence[~train_test_overlap_mask]
        inputs_mask = (
            inputs_mask[~train_test_overlap_mask] if inputs_mask is not None else None
        )
        targets_mask = (
            targets_mask[~train_test_overlap_mask] if targets_mask is not None else None
        )
        sample_weights = (
            tuple(
                [
                    ...
                    if not train_test_overlap_mask[i]
                ]
            )
    ...
    if data_given_grammar_smoothing_epsilon is not None:
        # Allow smoothing the log probabilities to avoid infinite D:G (useful for comparisons)
        log_probabilities = -np.log2(
            target_classes_probabs + data_given_grammar_smoothing_epsilon
        )
    else:
    ...
    for mini_batch in mini_batches:
        try:
            batch_encoding_length, batch_accuracy = _get_sequence_encoding_length(
                ...
                softmax_outputs=config.softmax_outputs,
                try_net_early_stop=config.try_net_early_stop,
                data_given_grammar_smoothing_epsilon=config.data_given_grammar_smoothing_epsilon,
                ...
                allow_test_overlap=config.allow_test_overlap,
                train_test_overlap_mask=mini_batch.train_test_overlap_mask,
            )
            ...
            logger.debug(f"Invalid network: {str(e)}")
            return _INF, 0.0
...
def _calculate_regularization(
    reg: float,
    weights: dict[tuple[int, ...], _Weight],
    config: configuration.SimulationConfig,
) -> float:
    if config.regularization_method == "l1":
        for edge, weight in weights.items():
            reg += abs(weight.numerator / weight.denominator)
    elif config.regularization_method == "l2":
        for edge, weight in weights.items():
            reg += (weight.numerator / weight.denominator) ** 2
    return reg
...
def calculate_fitness(
    ...
    reg = 0
    if config.regularization_method != "none":
        reg = _calculate_regularization(reg, net.forward_weights, config)
        reg = _calculate_regularization(reg, net.recurrent_weights, config)
        reg = config.regularization_multiplier * reg

    grammar_encoding_length = get_encoding_length(
        net=net,
    ...
    return dataclasses.replace(
        net,
        fitness=Fitness(
            loss=(config.grammar_multiplier * grammar_encoding_length)
...
def get_forward_weights(net: Network) -> np.ndarray:
    return np.array([get_weight_real_value(w) for w in net.forward_weights.values()])


def get_recurrent_weights(net: Network) -> np.ndarray:
    return np.array([get_weight_real_value(w) for w in net.recurrent_weights.values()])
...
def mutate(net: Network, config: configuration.SimulationConfig) -> Network:
    ...
    net_size = (
        net.fitness.grammar_encoding_length
        if net.fitness is not None
        else get_encoding_length(
            net=net,
            allowed_activations=config.allowed_activations,
            compress_encoding=config.compress_grammar_encoding,
        )
    )
    net_exceeds_max_grammar_size = (
        net_size >= config.max_grammar_size
        if config.max_grammar_size is not None
        else False
    )
    ...
    mutation_conditions = (
        (
            _add_random_unit,
            num_units < config.max_network_units
            and total_connections > 0
            and not net_exceeds_max_grammar_size
            and config.allow_architecture_changing_mutations,
        ),
        (
            _remove_random_unit,
            num_hidden_units > 0 and config.allow_architecture_changing_mutations,
        ),
        (
            _mutate_activation,
        ...
        (
            _mutate_unit_type,
            len(config.allowed_unit_types) > 1
            and (num_hidden_units + num_outputs) > 0
            and config.allow_architecture_changing_mutations,
        ),
        (
            _add_random_connection,
        ...
        (
            _add_random_bias,
            num_biases < (num_hidden_units + num_outputs)
            and not net_exceeds_max_grammar_size,
        ),
        (_remove_random_bias, num_biases > 0),
        (
            _mutate_weight,
            total_connections_including_biases > 0 and not net_exceeds_max_grammar_size,
        ),
    )
    valid_mutations = tuple(filter(lambda x: x[1], mutation_conditions))
    mutation_func = random.choice(valid_mutations)[0]

    logger.debug(f"Applying mutation {mutation_func.__name__}")
...
def _add_random_bias(net: Network, config: configuration.SimulationConfig) -> Network:
    ...
    logger.debug(f"Added bias {weight} to unit {target_unit}")
...
def _remove_random_bias(
    ...
    logger.debug(f"Remove bias from unit {target_unit}")
    return dataclasses.replace(net, biases=new_biases)
...
def _replace_connections_and_weights(
    ...
    replace_kwargs = dict(
        zip(
            replace_args,
            (new_connections, new_reverse_connections, new_weights),
        )
    )
...
def _mutate_unit_type(net: Network, config: configuration.SimulationConfig) -> Network:
    ...
    logger.debug(f"Mutated unit {target} type: {old_type}->{new_type}")
...
def get_dot_string(
    ...
    class_to_label: Optional[dict[int, str]] = None,
) -> str:
    activation_to_color = {
        LINEAR: "#f2e7f7",
        SIGMOID: "#ffeba9",
    ...
    for edge_type in (FORWARD_CONNECTION, RECURRENT_CONNECTION):
        ...
        for node in get_units(net):
            ...
            for neighbor in connections[node]:
                weight = get_weight_float(net, edge_type, node, neighbor)
...
def visualize(
    ...
    path = pathlib.Path(f"dot_files/{filename}.dot")
    ...
    if image_format:
        ...
        print(graphviz.render("dot", image_format, str(path)))


def save(net: Network, name: str):
...
def to_string(net: Network) -> str:
    if net.fitness is not None:
        fitness = f"{net.fitness.loss:,.2f}"
        g = f"{net.fitness.grammar_encoding_length:,.2f}"
        d_g = f"{net.fitness.data_encoding_length:,.5f}"
        reg = f"{net.fitness.regularization_term:,.2f}"
        accuracy = f"{net.fitness.accuracy:.2f}"
    else:
        fitness = g = d_g = accuracy = reg = "?"
...
def crossover(net1: Network, net2: Network, max_units: int = 10_000) -> Network:
    input_size = len(net1.input_units_range)
    output_size = len(net1.output_units_range)

    num_offspring_nodes = (
        get_num_units(net1) + get_num_units(net2) + input_size + output_size
    )
    if num_offspring_nodes >= max_units:
        return net1

    net1_offset = input_size + output_size
    net2_offset = net1_offset + get_num_units(net1)

    net1_forward_connections = _offset_all_node_ids_by(
        net1.forward_connections, offset=net1_offset
    )
    net1_forward_reverse_connections = _offset_all_node_ids_by(
        net1.reverse_forward_connections, offset=net1_offset
    )
    net1_forward_weights = _offset_all_node_ids_by(
        net1.forward_weights, offset=net1_offset
    )
    net1_recurrent_connections = _offset_all_node_ids_by(
        net1.recurrent_connections, offset=net1_offset
    )
    net1_recurrent_reverse_connections = _offset_all_node_ids_by(
        net1.reverse_recurrent_connections, offset=net1_offset
    )
    net1_recurrent_weights = _offset_all_node_ids_by(
        net1.recurrent_weights, offset=net1_offset
    )
    net1_biases = _offset_all_node_ids_by(net1.biases, offset=net1_offset)
    net1_activations = _offset_all_node_ids_by(net1.activations, offset=net1_offset)
    net1_unit_types = _offset_all_node_ids_by(net1.unit_types, offset=net1_offset)

    net2_forward_connections = _offset_all_node_ids_by(
        net2.forward_connections, offset=net2_offset
    )
    net2_forward_reverse_connections = _offset_all_node_ids_by(
        net2.reverse_forward_connections, offset=net2_offset
    )
    net2_forward_weights = _offset_all_node_ids_by(
        net2.forward_weights, offset=net2_offset
    )
    net2_recurrent_connections = _offset_all_node_ids_by(
        net2.recurrent_connections, offset=net2_offset
    )
    net2_recurrent_reverse_connections = _offset_all_node_ids_by(
        net2.reverse_recurrent_connections, offset=net2_offset
    )
    net2_recurrent_weights = _offset_all_node_ids_by(
        net2.recurrent_weights, offset=net2_offset
    )
    net2_biases = _offset_all_node_ids_by(net2.biases, offset=net2_offset)
    net2_activations = _offset_all_node_ids_by(net2.activations, offset=net2_offset)
    net2_unit_types = _offset_all_node_ids_by(net2.unit_types, offset=net2_offset)

    new_forward_connections = {**net1_forward_connections, **net2_forward_connections}
    new_forward_reverse_connections = {
        **net1_forward_reverse_connections,
        **net2_forward_reverse_connections,
    }
    new_forward_weights = {**net1_forward_weights, **net2_forward_weights}
    new_recurrent_connections = {
        **net1_recurrent_connections,
        **net2_recurrent_connections,
    }
    new_recurrent_reverse_connections = {
        **net1_recurrent_reverse_connections,
        **net2_recurrent_reverse_connections,
    }

    new_recurrent_weights = {**net1_recurrent_weights, **net2_recurrent_weights}
    new_biases = {**net1_biases, **net2_biases}
    new_activations = {**net1_activations, **net2_activations}
    new_unit_types = {**net1_unit_types, **net2_unit_types}

    # Connect input source to both net's inputs
    for input_unit in net1.input_units_range:
        new_forward_connections[input_unit] = frozenset([input_unit + net1_offset])
        if input_unit + net1_offset not in new_forward_reverse_connections:
            new_forward_reverse_connections[input_unit + net1_offset] = frozenset()
        new_forward_reverse_connections[input_unit + net1_offset] |= {input_unit}

        new_forward_connections[input_unit] |= {input_unit + net2_offset}
        if input_unit + net2_offset not in new_forward_reverse_connections:
            new_forward_reverse_connections[input_unit + net2_offset] = frozenset()
        new_forward_reverse_connections[input_unit + net2_offset] |= {input_unit}

        new_forward_weights[(input_unit, input_unit + net1_offset)] = _IDENTITY_WEIGHT
        new_forward_weights[(input_unit, input_unit + net2_offset)] = _IDENTITY_WEIGHT

        new_activations[input_unit] = LINEAR
        new_unit_types[input_unit] = SUMMATION_UNIT

    # Connect output sink from both nets' outputs
    for output_unit in net1.output_units_range:
        output_in_net1 = output_unit + net1_offset
        output_in_net2 = output_unit + net2_offset

        if output_in_net1 not in new_forward_connections:
            new_forward_connections[output_in_net1] = frozenset()
        new_forward_connections[output_in_net1] |= {output_unit}

        if output_in_net2 not in new_forward_connections:
            new_forward_connections[output_in_net2] = frozenset()
        new_forward_connections[output_in_net2] |= {output_unit}

        new_forward_reverse_connections[output_unit] = frozenset(
            [output_in_net1, output_in_net2]
        )

        # Average outputs from both nets into offspring outputs.
        weight_to_output = _Weight(1, 1, output_size)
        new_forward_weights[(output_in_net1, output_unit)] = weight_to_output
        new_forward_weights[(output_in_net2, output_unit)] = weight_to_output

        new_activations[output_unit] = LINEAR
        new_unit_types[output_unit] = SUMMATION_UNIT

    return Network(
        input_units_range=net1.input_units_range,
        output_units_range=net1.output_units_range,
        forward_weights=new_forward_weights,
        recurrent_weights=new_recurrent_weights,
        forward_connections=new_forward_connections,
        reverse_forward_connections=new_forward_reverse_connections,
        recurrent_connections=new_recurrent_connections,
        reverse_recurrent_connections=new_recurrent_reverse_connections,
        biases=new_biases,
        activations=new_activations,
        unit_types=new_unit_types,
    )
...
def import_gaier_and_ha_net(
    weight_agnostic_net_path: str,
    trained_weights_net_path: str,
    corpus: corpora.Corpus,
) -> Network:
    # weight_agnostic file:
    #   ind = np.loadtxt(fileName, delimiter=',')
    #   wMat = ind[:,:-1]     # Weight Matrix
    #   aVec = ind[:,-1]      # Activation functions
    #
    # json file: list of active weight values

    gaier_and_ha_activations_to_ours = {
        1: LINEAR,
        2: UNSIGNED_STEP,
        3: _SINUS,
        4: _GAUSSIAN,
    ...
    weight_agnostic = np.loadtxt(weight_agnostic_net_path, delimiter=",")
    connections = weight_agnostic[:, :-1]
    activations = weight_agnostic[:, -1]

    weight_vector = connections.flatten()
    weight_vector[np.isnan(weight_vector)] = 0
    weights_key = np.where(weight_vector != 0)[0]

    with open(trained_weights_net_path) as f:
        flat_weights = json.load(f)[0]

    num_connections = len(flat_weights)

    for i in range(num_connections):
        key = weights_key[i]
        weight_vector[key] = flat_weights[i]

    for i in range(num_connections):
        key = weights_key[i]
        weight_vector[key] = flat_weights[i]

    num_units_including_bias_unit = len(activations)
    weights = weight_vector.reshape(
        (num_units_including_bias_unit, num_units_including_bias_unit)
    )

    input_size = corpus.input_sequence.shape[-1]
    output_size = corpus.target_sequence.shape[-1]

    # G&H keep output units in the last rows of the connection matrix, we keep them right after the input rows.
    original_output_rows = list(range(weights.shape[0] - output_size, weights.shape[0]))
    new_output_rows = list(range(1 + input_size, 1 + input_size + output_size))

    buffer = np.copy(weights)

    weights[new_output_rows] = weights[original_output_rows]
    weights[original_output_rows] = buffer[new_output_rows]
    ...
    activations_buffer = np.copy(activations)
    activations[new_output_rows] = activations[original_output_rows]
    activations[original_output_rows] = activations_buffer[new_output_rows]

    # Unit 0 is the bias source.
    activations = list(activations[1:])
    num_units = len(activations)
    biases = [weights[0, i] for i in range(1, num_units_including_bias_unit)]
    weights = weights[1:, 1:]

    logger.info(f"Imported net num units: {num_units}")
    logger.info(f"Imported net num connections: {len(flat_weights)}")

    weights_dict = collections.defaultdict(tuple)

    for source_unit in range(num_units):
        for target_unit in np.where(weights[source_unit])[0]:
            w = weights[source_unit, target_unit]
            sign = np.sign(w)
            fraction = fractions.Fraction(abs(w)).limit_denominator(10)
            if fraction.numerator == 0:
                # TODO: this isn't necessarily what we want, i.e. we may want to penalize redundant weights.
                continue

            weights_dict[source_unit] += (
                (target_unit, sign, fraction.numerator, fraction.denominator),
            )

    return make_custom_network(
        corpus.input_sequence.shape[0],
        corpus.target_sequence.shape[-1],
        num_units,
        forward_weights=weights_dict,
        recurrent_weights={},
        activations={
            unit: gaier_and_ha_activations_to_ours[x]
            for unit, x in enumerate(activations)
        },
        biases=dict(enumerate(biases)),
    )

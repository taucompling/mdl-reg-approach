...
class VanillaRNN(nn.Module):
    def __init__(
        ...
        self._rnn_layer = rnn_layer(**rnn_kwargs, device=utils.DEVICE)

        layers = [
            nn.Linear(
                in_features=self._num_hidden_units,
        ...
        logger.info(f"Using device {utils.DEVICE}")
    ...
    def feed_sequence(self, input_sequence):
        with torch.no_grad():
            return self._forward(input_sequence.to(utils.DEVICE))
...
def calculate_symbolic_accuracy(
    ...
    predicted_probabs_packed = net.feed_sequence(inputs_packed).cpu()
    predicted_probabs = _unpack(
        ...
        num_classes=valid_targets_mask.shape[-1],
    )
...
def evaluate(
    ...
    inputs_packed = _pack_masked(inputs, sequence_lengths).to(utils.DEVICE)
    ...
    y_pred = net.feed_sequence(inputs_packed).to("cpu")
    ...
    if deterministic_steps_mask is not None:
        ...
        det_accuracy = float(
            det_correct_weighted.sum() / det_flat_sample_weights.sum().item()
        )
    else:
        det_accuracy = None

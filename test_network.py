...
def _test_net_on_deterministic_task(
    net: network.Network,
    corpus: corpora.Corpus,
    config: configuration.SimulationConfig,
    assert_perfect: bool = False,
    ...
    if net.fitness is not None:
        # Should be the same network, if not then problem with config or pickling
        assert np.isclose(
            train_net.fitness.grammar_encoding_length,
            net.fitness.grammar_encoding_length,
        ).all()
        assert np.isclose(
            train_net.fitness.data_encoding_length, net.fitness.data_encoding_length
        ).all()

    train_det_acc = network.calculate_deterministic_accuracy(
        train_net, train_corpus, config=config
    )
    if assert_perfect:
        assert train_det_acc == 1.0
    ...
    logger.info(test_net_no_overlap)

    test_det_acc = network.calculate_deterministic_accuracy(
        test_net, test_corpus, config=config
    )
    if assert_perfect:
        assert test_det_acc == 1.0

    if should_visualize:
        network.visualize(
            train_net, corpus.name, class_to_label=corpus.vocabulary, image_format="png"
        )

    logger.info(f"Results for corpus {corpus.name}:")
    logger.info(
        f"Train |G|: {train_net.fitness.grammar_encoding_length}, Train |D:G|: {train_net.fitness.data_encoding_length}"
    )
    logger.info(f"Train deterministic accuracy: {train_det_acc}")
    logger.info(
        f"Test |G|: {test_net.fitness.grammar_encoding_length}, Test |D:G|: {test_net.fitness.data_encoding_length}"
    )
    logger.info(
        f"Test |D:G| (no overlap): {test_net_no_overlap.fitness.data_encoding_length}"
    )
    logger.info(f"Test deterministic accuracy: {test_det_acc}")


def _test_net_on_symbolic_task(
    net: network.Network,
    corpus: corpora.Corpus,
    train_valid_targets: np.ndarray,
    test_valid_targets: np.ndarray,
    config: configuration.SimulationConfig,
    assert_perfect: bool = False,
    should_visualize: bool = False,
) -> None:
    ...
    logger.info("Calculating train fitness...")
    train_corpus = corpora.optimize_for_feeding(corpus)
    train_net = network.calculate_fitness(invalidated_net, train_corpus, config=config)
    logger.info(train_net)
    ...
    train_symbolic_acc, _ = network.calculate_symbolic_accuracy(
        valid_targets_mask=train_valid_targets,
        found_net=train_net,
        inputs=train_corpus.input_sequence,
        input_mask=train_corpus.input_mask,
        sample_weights=train_corpus.sample_weights,
        config=config,
        epsilon=0.005,
        plots=False,
    )

    logger.info(f"Train symbolic accuracy: {train_symbolic_acc}")
    if assert_perfect:
        assert train_symbolic_acc == 1.0

    logger.info("Calculating test fitness...")
    test_corpus = corpora.optimize_for_feeding(corpus.test_corpus)
    test_net = network.calculate_fitness(invalidated_net, test_corpus, config=config)
    logger.info(test_net)
    ...
    test_symbolic_acc, _ = network.calculate_symbolic_accuracy(
        valid_targets_mask=test_valid_targets,
        found_net=test_net,
        inputs=test_corpus.input_sequence,
        input_mask=test_corpus.input_mask,
        sample_weights=test_corpus.sample_weights,
        config=config,
        epsilon=0.005,
        plots=False,
    )
    ...
    logger.info(f"Test symbolic accuracy: {test_symbolic_acc}")
...
def _evolve_weight(
    ...
    while i < max_retries:
        if network.get_weight_real_value(w) == network.get_weight_real_value(
            target_weight
        ):
            logger.info(
                f"Weight {network.get_weight_real_value(w)} generation took {i} steps, started from {network.get_weight_real_value(initial_w):.2f}"
            )
            return w
        w = network._perturbate_weight(w)
        i += 1
    raise ValueError(
        f"Couldn't generate weight {network.get_weight_real_value(target_weight)}, started from {network.get_weight_real_value(initial_w)}"
    )
...
class TestNetwork(unittest.TestCase):
    def test_topological_sort(self):
        ...
        loop_edges, topological_sort = network.dfs(
    ...
    def test_crossover(self):
        ...
        net1 = network.make_custom_network(
            ...
            biases={2: 2, 5: 5},
            recurrent_weights={5: ((2, 1, net1_weight + 1, 1),)},
            activations={i: network.SQUARE for i in range(net1_num_units)},
        )
        network.visualize(net1, "crossover_net1")
        ...
        net2 = network.make_custom_network(
            ...
            biases={3: 3, 6: 6},
            recurrent_weights={},
            activations={i: network.SIGMOID for i in range(net2_num_units)},
        )
        network.visualize(net2, "crossover_net2")

        offspring = network.crossover(net1, net2)
        network.visualize(offspring, "crossover_offspring")

        assert network.get_num_units(offspring) == net1_num_units + net2_num_units + 5
    ...
    def test_equality(self):
        ...
        net3 = dataclasses.replace(net1, fitness=network.Fitness(1, 2, 3, 4, 0))
        assert net1 == net2
        assert net1 == net3
    ...
    def test_topological_depth_stack_net(self):
        net = manual_nets.make_emmanuel_dyck_2_network_io_protection(nesting_probab=0.3)
        bfs_layers = network.bfs_layers(
        ...
        bfs = {x: sorted(y) for x, y in bfs_layers.items()}
        network.visualize(net, "test_topological_depth_stack_net")
        print(json.dumps(bfs, indent=1))

    def test_topological_depth(self):
        net = network.make_custom_network(
            input_size=3,
            output_size=1,
            num_units=9,
        ...
        network.visualize(net, "test_topological_depth")
        bfs_layers = network.bfs_layers(
        ...
        for depth, units in target_layers.items():
            assert bfs_layers[depth] == units
    ...
    def test_loop(self):
        ...
        loop_edges, topological_order = network.dfs(
            units=network.get_units(net),
            reverse_connections=net.reverse_forward_connections,
            input_units=net.input_units_range,
            output_units=net.output_units_range,
    ...
    def test_binary_output(self):
        ...
        target_encoding_length = -np.sum(
            np.log2(
                [
                    network._sigmoid(weight_val),
                    1 - network._sigmoid(weight_val),
                ]
            )
        )
    ...
    def test_conll_addition_network(self):
        ...
        assert net.fitness.accuracy == 1.0
        assert net.fitness.grammar_encoding_length == 109
    ...
    def test_emmanuel_an_bn_network(self):
        ...
        print(net)
        # TODO: Add correct assertion for data_encoding_length after fixing corpora creation bugs and calculations
    ...
    def test_an_bn_handmade_network(self):
        ...
        outputs = corpora.an_bn_handmade_net(
            anbn_corpus.input_sequence,
            prior=prior,
        )
    ...
    def test_recurrent_loop(self):
        ...
        print(net)
        assert np.isinf(net.fitness.data_encoding_length)
        assert net.fitness.grammar_encoding_length == 39

    @unittest.skip("Need to import the G&H net from their project")
    def test_gaier_and_ha_mnist_net(self):
        corpus = corpora.make_mnist_corpus(samples_limit=1000, width_height=16)
        weight_agnostic_net_path = (
            "./network_zoo/external-nets/gaier-and-ha-mnist/mnist.out"
        )
        trained_weights_net_path = (
            "./network_zoo/external-nets/gaier-and-ha-mnist/mnist256.wann.json"
        )
        net = network.import_gaier_and_ha_net(
            weight_agnostic_net_path, trained_weights_net_path, corpus
        )
        network.visualize(net, "gaier_ha_mnist", hide_non_connected_units=True)

        config = dataclasses.replace(
            _TEST_CONFIG, softmax_outputs=True, truncate_large_values=False
        )

        test_corpus = dataclasses.replace(
            corpus,
            input_sequence=corpus.test_input_sequence,
            target_sequence=corpus.test_target_sequence,
        )

        net = network.calculate_fitness(net, test_corpus, config)
        print(net)

        assert net.fitness.accuracy >= 0.94
    ...
    def test_multiplication_unit_with_bias(self):
        ...
        print(outputs)
        assert np.all(outputs.flatten() == [3, 8, 18, 38])
    ...
    def test_emmanuel_dyck_2_net(self):
        ...
        net_with_io_units_protected = (
            manual_nets.make_emmanuel_dyck_2_network_io_protection(
                nesting_probab=nesting_probab
            )
        )
    ...
    def test_tacl_an_bn_net(self):
        config = _TEST_CONFIG
        utils._set_seed_to_corpus_seed(config)

        corpus = corpora.make_an_bn(batch_size=500, prior=0.3, sort_by_length=False)
        corpus = corpora.compute_train_test_overlap_mask(
            corpus, is_exhaustive_test_corpus=True
        )
        net = manual_nets.make_tacl_paper_an_bn_net()
        _test_net_on_deterministic_task(
            net, corpus, config, assert_perfect=True, should_visualize=False
        )

    def test_tacl_an_bn_cn_net(self):
        config = _TEST_CONFIG
        utils._set_seed_to_corpus_seed(config)
        ...
        net = manual_nets.make_tacl_paper_an_bn_cn_net()
        _test_net_on_deterministic_task(
            net, corpus, config, assert_perfect=True, should_visualize=False
        )

    def test_tacl_dyck_1_net(self):
        config = _TEST_CONFIG
        utils._set_seed_to_corpus_seed(config)

        corpus = corpora.make_dyck_n(
            n=1, batch_size=500, nesting_probab=0.3, max_sequence_length=200
        )
        corpus = corpora.compute_train_test_overlap_mask(
            corpus, is_exhaustive_test_corpus=True
        )
        net = manual_nets.make_tacl_paper_dyck_1_net()
        train_valid_targets = corpora.get_dyck_valid_targets(corpus, 1)
        test_valid_targets = corpora.get_dyck_valid_targets(corpus.test_corpus, 1)

        _test_net_on_symbolic_task(
            net,
            corpus,
            train_valid_targets,
            test_valid_targets,
    ...
    def test_tacl_dyck_2_net(self, nesting_probability: float = 0.33333):
        config = _TEST_CONFIG
        utils._set_seed_to_corpus_seed(config)
        ...
        corpus = corpora.make_dyck_n(
            n=2,
            batch_size=500,
            nesting_probab=nesting_probability,
            max_sequence_length=200,
        )
        corpus = corpora.compute_train_test_overlap_mask(
            corpus, is_exhaustive_test_corpus=True
        ...
        # Make sure to use extended unit types and extended activations.
        config = dataclasses.replace(
            config,
            allowed_unit_types=EXTENDED_UNIT_TYPES,
            allowed_activations=EXTENDED_ACTIVATIONS,
        )

        net = manual_nets.make_tacl_paper_dyck_2_net(nesting_probab=nesting_probability)
        train_valid_targets = corpora.get_dyck_valid_targets(corpus, 2)
        test_valid_targets = corpora.get_dyck_valid_targets(corpus.test_corpus, 2)
